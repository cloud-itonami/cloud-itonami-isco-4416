(ns personnelclerk.actor
  "PersonnelClerksActor — the ISCO-08 4416 community personnelclerk actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one personnelclerk operation request
  ...
  The unconditional invariant: the PersonnelClerksAdvisor can never
  directly commit a record the PersonnelClerksGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [personnelclerk.advisor :as advisor]
            [personnelclerk.governor :as governor]
            [personnelclerk.store :as store]))

(defn build-graph
  "Build a compiled PersonnelClerksActor graph. `store` implements
  `personnelclerk.store/Store`. `advisor` implements
  `personnelclerk.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :position-id (:position-id proposal)
                                    :payload proposal}]
